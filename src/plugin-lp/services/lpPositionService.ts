...
export class LPPositionService implements ILPPositionService {
  ...
  private async callWithRateLimit<T>(apiCall: () => Promise<T>): Promise<T> {
    const now = Date.now();
    const timeSinceLastCall = now - this.lastApiCallTime;

    if (timeSinceLastCall < this.API_CALL_INTERVAL) {
      await new Promise((resolve) =>
        setTimeout(resolve, this.API_CALL_INTERVAL - timeSinceLastCall)
      );
    }
  ...
  private async fetchUniswapPositions(walletAddress: string): Promise<any[]> {
    try {
      // First try the subgraph
      try {
        const query = gql`
        ...
        const data = await this.callWithRateLimit(() =>
          request(UNISWAP_V3_SUBGRAPH, query, {
            owner: walletAddress.toLowerCase(),
          })
        );

        if ((data as any).positions?.length > 0) {
          elizaLogger.log(
            `Found ${(data as any).positions.length} positions via subgraph`
          );
          return (data as any).positions;
        }
      } catch (subgraphError) {
        elizaLogger.error(`Subgraph query failed: ${subgraphError}`);
      }

      // If subgraph fails, try Alchemy NFT API (Uniswap positions are NFTs)
      if (this.provider && this.alchemyKey) {
        elizaLogger.log("Trying Alchemy NFT API for Uniswap positions");
        ...
        if (nftData.ownedNfts && nftData.ownedNfts.length > 0) {
          ...
          // Convert Alchemy NFT data to our position format
          // This would require additional processing in a real implementation
          return [];
        }
      }

      elizaLogger.warn("No positions found through any method");
      return [];
    } catch (error) {
      elizaLogger.error(`Error fetching Uniswap positions: ${error}`);
      return [];
    }
  }
  ...
  private async calculatePositionsPnl(positions: any[]): Promise<LPPosition[]> {
    ...
    for (const position of positions) {
      try {
        ...
        const token0 = new Token(
          1, // Ethereum mainnet
          position.token0.id,
          parseInt(position.token0.decimals),
        ...
        const token1 = new Token(
          1, // Ethereum mainnet
          position.token1.id,
          parseInt(position.token1.decimals),
          position.token1.symbol
        );

        // Get current token prices from a price oracle (e.g., CoinGecko)
        const [token0Price, token1Price] = await Promise.all([
          this.getTokenPrice(token0.address),
          this.getTokenPrice(token1.address),
        ]);
        ...
        // Get current amounts
        const { amount0, amount1 } = positionInstance.mintAmounts;
        const currentAmount0 = ethers.formatUnits(
          amount0.toString(),
          position.token0.decimals
        );
        const currentAmount1 = ethers.formatUnits(
          amount1.toString(),
          position.token1.decimals
        );

        // Get deposited amounts
        const depositedAmount0 = ethers.formatUnits(
          position.depositedToken0,
          position.token0.decimals
        );
        const depositedAmount1 = ethers.formatUnits(
        ...
        // Calculate PnL
        const token0Diff =
          parseFloat(currentAmount0) - parseFloat(depositedAmount0);
        const token1Diff =
          parseFloat(currentAmount1) - parseFloat(depositedAmount1);

        const depositedValueUsd =
          parseFloat(depositedAmount0) * token0Price +
          parseFloat(depositedAmount1) * token1Price;

        const currentValueUsd =
          parseFloat(currentAmount0) * token0Price +
          parseFloat(currentAmount1) * token1Price;

        const pnlUsd = currentValueUsd - depositedValueUsd;
        const pnlPercentage = (pnlUsd / depositedValueUsd) * 100;
        ...
        result.push({
          ...
          pnl: {
            token0:
              token0Diff > 0
                ? `+${token0Diff.toFixed(6)}`
                : token0Diff.toFixed(6),
            token1:
              token1Diff > 0
                ? `+${token1Diff.toFixed(6)}`
                : token1Diff.toFixed(6),
            usd: pnlUsd > 0 ? `+${pnlUsd.toFixed(2)}` : pnlUsd.toFixed(2),
            percentage:
              pnlPercentage > 0
                ? `+${pnlPercentage.toFixed(2)}%`
                : `${pnlPercentage.toFixed(2)}%`,
        ...
        elizaLogger.error(
          `Error calculating PnL for position ${position.id}: ${error}`
        );
      }
    }
  ...
  private async getTokenPrice(tokenAddress: string): Promise<number> {
    try {
      // In a real implementation, you would use a price oracle like CoinGecko
